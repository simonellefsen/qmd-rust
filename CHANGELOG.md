...
- Land large pending Iteration 2/3 base (user-directed after clean v0.6.8 hygiene release): full review + docs-only landing of the pre-existing base material (4 proper per-command modules: bench, init, multi_get, skill + real reranker for_rerank + cosine + chunk-strategy auto wiring + I3 polish). 0 Rust source changes required (base already high-fidelity and pattern-compliant). Wiki-first entries + decision record update + CHANGELOG. Internal + external review to documented 0 open code issues (2 sound wontfixes; 1 pre-existing distribution bug in skill surface + low-sev nits tracked for post-landing follow-up). Full reinforced gates clean on dirty tree. Large pending I2/I3 base is now finished and ready for commit. (IMPL_ID 7b9deaa0)
...
- Post-I3 gaps #3/#4 progress (two smallest sub-slices, wiki-first log + decision before .rs and before each major step; continuing exact discipline from #1/#2):
  - #3 Production-grade chunking (`--chunk-strategy auto`): smallest viable strengthen of the existing std-only marker skeleton (in src/index/mod.rs only) by extending auto strategy with Python/Go/Markdown header support + richer markers for covered languages. Doc updates in src/cli/args.rs ChunkStrategy. No new dependencies (tree-sitter + grammars avoided per "no heavy new" + "smallest viable" constraints; graceful fallback + fp unchanged). Separate wiki entries for sub-slice start + completion. Full fmt/clippy/check/test gates clean after change + fmt normalize. Delivers better code chunking for agent/llm-wiki knowledge bases using only existing patterns.
  - #4 (in progress in next sub-slice): richer per-collection embedding health, fingerprint status, model diagnostics, vector store health surfaced in `status` (text + JSON) and `cleanup`, via tiny reusable helpers in db/ + edits to status.rs only.
  - #4 complete (second sub-slice): smallest viable richer observability. Added `collection_vector_count` helper in db/mod.rs (exact pattern reuse of get_collection_stats + open_connection + query_row + newbie comment). Enhanced status.rs (use update, per-coll health loop via cfg+helpers, global distinct fp/model queries + vec store health note + mixed-fp warning, extended JSON keys + text output for fingerprints/models/per-coll/vec health). 1 clippy lint fix only. No cleanup.rs touch (kept smallest), no new files/monoliths. Wiki-first entries + decision + this changelog. Full gates (fmt + 2x clippy -D + check + test 15/15) clean after. Delivers per-collection embedding coverage + fp consistency visibility for agents/llm-wiki maintenance loops.
...
- Post-landing gap (highest-priority remaining after #1–#4 + v0.6.9 I2/I3 base landing, IMPL_ID afaa49a6): fix pre-existing distribution bug in skill surface (explicitly tracked in 0.6.9 changelog + 7b9deaa0 review notes for post-landing follow-up; previously defended wontfix while base pending). Wiki-first (log.md entry + decision record update) as absolute first actions before any .rs. Smallest viable slice (one file only): embed the skill assets (SKILL.md + references/mcp-setup.md) at compile time via `include_str!` (builtin, zero new deps/crates) in src/cli/commands/skill.rs; removed the 3 dead runtime-FS helpers (bundled_skill_dir using CARGO_MANIFEST_DIR + exists check, read_skill_md, copy_dir_contents); rewrote cmd_skill/cmd_skills handlers to use the consts for all bundled content/writes (show/get print embedded; install creates dirs + writes stub + ref asset; list/path report generically without "not found"). Full fidelity to patterns (newbie comments on includes/consts, reuse of stub fn, fs idioms). Skill show/install/skills* now work from any installed binary (cargo install, cargo-dist releases, etc.) while dev `cargo run` behavior unchanged. Updated module docs for accuracy. Zero external path refs in new text/code/comments/artifacts (generic "embedded in this binary", "bundled skill assets (embedded at compile time)" only). Review loop via /tmp/grok-review-afaa49a6.md to 0 open (1 high-sev fixed, others wontfix with defenses for scope/precedent). Full reinforced gates (fmt --check + 2x clippy -D + test --all + check) clean immediately before summary. Touched: wiki/log.md, wiki/decisions/2026-05-next-parity-phases.md, src/cli/commands/skill.rs, CHANGELOG.md, review + summary artifacts. Closes the tracked post-landing item; advances fidelity/edge parity for the landed I1 commands and drop-in replacement goal for agents/llm-wiki. Continues exact controlled discipline of prior gaps + landing.
- Landing of large pending Iteration 2/3 base (IMPL_ID 7b9deaa0, post clean v0.6.8 release): user-directed finish of the long-pending material kept exactly uncommitted through prior slices and the v0.6.8 hygiene. Wiki-first (log.md x2 + decision record update) before any changes. Full review of the base (4 untracked proper per-command modules + wiring diffs) confirmed: follows all existing patterns exactly (newbie comments, helper reuse, no monoliths, one-cmd-per-file), zero external path references in code/comments (grepped + cleaned in base diffs), passes reinforced gates clean (fmt --check, 2x clippy -D all-targets, cargo test --all 15/15). No Rust source changes needed in landing run (base quality high); only docs (this changelog, wiki entries). User-visible: `qmd init`, `qmd bench`, `qmd skill show/install`, `qmd skills list/get/path`, `qmd multi-get` now fully functional via dedicated modules (removed from any catch-all); real reranker (`models.rerank` / QMD_RERANK_MODEL via for_rerank + cosine post-fusion in query), multi-vec auto-expansion, --chunk-strategy auto (std skeleton) fully wired and active. Touched for this phase: wiki/log.md (x2), wiki/decisions/2026-05-next-parity-phases.md, CHANGELOG.md (plus the 4 command modules + 5 modified files from base material, to be git-added by orchestrator). Full gates re-run clean at end. Review notes file used for self-audit loop (0 open code issues; 1 style precedent defended as wontfix). All AGENTS.md rules + historical contract (override only for this authorized landing) + zero external refs + no real index mutations observed. Prepares the foundation for future gaps; orchestrator will commit the base + tag after 0-issue review.
- Deeper MCP surface (gap #1 post-I3, highest priority for agent/llm-wiki): wiki-first (log.md + decision record update before any .rs); smallest viable in src/cli/commands/mcp.rs only. Added `structuredContent` to tools/call results (status, query results as {docid,file,title,score,snippet} arrays, get/multi_get metadata) for reliable structured agent parsing (no text scraping). Enriched tool schemas + long agent-oriented descriptions (query grammar, examples, params like min_score/intent/fromLine). Made multi_get functional for comma lists (reuses get_body_from_db; glob deferred). Minor query/get enhancements (FtsHit fields, fromLine/maxLines aliases, min_score filter). Updated startup. Followed patterns, maximal reuse, no new deps/files, zero outside-workspace refs in new text. Full gates clean. See wiki/log.md for slice rationale + wontfix defenses (e.g. no hybrid vec in MCP this slice; CLI provides full power). Touched: wiki/log.md, wiki/decisions/2026-05-next-parity-phases.md, CHANGELOG.md, src/cli/commands/mcp.rs. Prepares reviewable progress on MCP parity for agents.

- v0.6.4 patch (minimal rescue): Made the crate compile and pass `cargo test --all` + full clippy again on the committed tree. Added the missing `ChunkStrategy` (Regex/Auto) enum + command fields, `default_reranker()` helper (for real rerank path), and `candidate_limit` support that were present locally but not captured in v0.6.2/v0.6.3 commits. Also fixed the recurring multiline `).unwrap();` fmt style in the hermetic test in `src/index/mod.rs` (the exact issue that failed v0.6.1 CI). Only minimal files staged; large pending work left uncommitted. Full gates clean before commit. Annotated tag v0.6.4.
- Release process hygiene: `cargo fmt --all -- --check` + `cargo clippy -- -D warnings` (and with `--features llama-embed`) are now explicitly documented as a mandatory gate in `wiki/runbooks/release.md` immediately before any release commit, annotated tag, or tag push. This was triggered by the v0.6.1 CI failure (`cargo fmt` rejected the multiline `).unwrap();` style on the `execute_batch` schema bootstrap inside the hermetic integration test in `src/index/mod.rs`). Current tree passes cleanly; future releases (0.6.3+, etc.) will not regress the same class of runner vs. local rustfmt drift.
