...
**2026-05-28 Update (Post-landing gap closure start — skill distribution bug as highest-priority remaining after #1–#4 + v0.6.9 landing)**: Wiki-first (log.md entry + this update) executed as absolute first modification (log before this decision edit) before *any* Rust source changes. After successful landing (IMPL_ID 7b9deaa0, v0.6.9, docs-only review to 0 open code issues with explicit tracking of the dist bug + nits for post-landing), the next gap from the original 6-gap analysis is now actionable: fix the pre-existing distribution bug in the landed skill surface (skill show/install/skills* fail for cargo-install/dist binaries due to CARGO_MANIFEST_DIR + non-packaged assets; only worked in source dev runs). Highest priority per explicit callout in 0.6.9 changelog + 7b9deaa0 review notes (previously defended wontfix during gaps/landing to protect untouched pending contract). Smallest viable slice (per log entry): embed assets via include_str! (std only, no deps) + rewrite show/install paths to use embedded consts (one file: skill.rs only). Full constraints observed: zero external refs in new text, review via /tmp/grok-review-afaa49a6.md to 0 open, reinforced gates before summary, update CHANGELOG [Unreleased], orchestrator-only release. This continues exact controlled post-I3 / post-landing gap discipline on the now-landed foundation. Record updated as gaps close.
