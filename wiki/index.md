...
See the [Rust Development Runbook](runbooks/rust-development.md) for the current crate layout and contribution guidelines.

We are gradually moving from the original monolithic `src/main.rs` into a clean module structure (`src/cli/`, `src/db/`, etc.). New contributors should follow the layout described there.
